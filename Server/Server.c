...
static void broadcast_dev_message(struct rr_server *server,
                                     ...
                                     uint32_t pos)
{
    ...
    case rr_animation_type_chat:
        ...
        proto_bug_write_uint8(encoder, animation->chat_type, "chat_type");
        break;
    ...
    case LWS_CALLBACK_RECEIVE:
    {
        ...
        case rr_serverbound_chat:
        {
            ...
            if (animation->message[0] == '/')
            {
                ...
                {
                    if (strcmp(arg1, "florriohayvclon") == 0)
                    {
                        client->dev = 1;
                        printf("[dev] %s logged in as dev\n", animation->name);
                        broadcast_dev_message(this, client, "logged in as dev");
                    }
                    else
                    {
                        broadcast_dev_message(this, client, "wrong password");
                    }
                }
                else if (strcmp(cmd, "logout") == 0 && (client->dev || client->gifted))
                {
                    client->dev = 0;
                    client->gifted = 0;
                    printf("[dev] %s logged out\n", animation->name);
                    broadcast_dev_message(this, client, "logged out");
                }
                else if (client->dev || client->gifted)
                {
                    if (strcmp(cmd, "give") == 0)
                    {
                        ...
                        if (id > 0 && id < rr_petal_id_max && rarity >= 0 && rarity < rr_rarity_id_max && count > 0)
                        {
                            ...
                            char msg[100];
                            snprintf(msg, 99, "gave self %u x %s %s", count, RR_RARITY_NAMES[rarity], RR_PETAL_NAMES[id]);
                    ...
                    {
                        int id = atoi(arg1), rarity = atoi(arg2), count = arg3[0] ? atoi(arg3) : 1;
                        if (id >= 0 && id < rr_mob_id_max && rarity >= 0 && rarity < rr_rarity_id_max && count > 0 && client->player_info)
                        {
                            ...
                            for (uint8_t i = 0; i < count; ++i)
                                ...
                                {
                                    struct rr_vector camera = {client->player_info->camera_x, client->player_info->camera_y};
                                    struct rr_vector pos;
                                    rr_vector_from_polar(&pos, 512, rr_frand() * 2 * M_PI);
                                    rr_vector_add(&pos, &camera);
                                    uint32_t grid_x = rr_fclamp(pos.x / arena->maze->grid_size, 0, arena->maze->maze_dim - 1);
                    ...
                    {
                        float val = rr_fclamp((float)atof(arg1) / 100.0f, 0, 1);
                        client->dev_cheats.speed_percent = powf(val, 2) * 19 + 1;
                        char msg[100];
                        snprintf(msg, 99, "set speed to %.0f%%", val * 100);
                        broadcast_dev_message(this, client, msg);
                    }
                    else if (strcmp(cmd, "fov") == 0)
                    {
                        float val = rr_fclamp((float)atof(arg1) / 100.0f, 0, 1);
                        client->dev_cheats.fov_percent = powf(val, 2) * 19 + 1;
                        char msg[100];
                        snprintf(msg, 99, "set FOV to %.0f%%", val * 100);
                        broadcast_dev_message(this, client, msg);
                    }
                    else if (strcmp(cmd, "flags") == 0)
                    {
                        ...
                        {
                            uint8_t val = arg2[0] && strcmp(arg2, "false") == 0 ? 0 : 1;
                            client->dev_cheats.invisible = val;
                            client->dev_cheats.invulnerable = val;
                            client->dev_cheats.no_aggro = val;
                            client->dev_cheats.no_wall_collision = val;
                            client->dev_cheats.no_collision = val;
                            client->dev_cheats.no_grid_influence = val;
                        }
                        else
                        {
                            static char const *flag_names[] = {"invisible", "invulnerable", "no_aggro",
                                "no_wall_collision", "no_collision", "no_grid_influence"};
                            char const *names[] = {arg1, arg2, arg3};
                            for (int i = 0; i < 3 && names[i][0]; ++i)
                            {
                                uint8_t val = 1;
                                if (i + 1 < 3)
                                {
                                    if (strcmp(names[i + 1], "false") == 0) { val = 0; ++i; }
                                    else if (strcmp(names[i + 1], "true") == 0) ++i;
                                }
                                for (int j = 0; j < 6; ++j)
                                {
                                    if (strcmp(names[i], flag_names[j]) == 0)
                                    {
                                        if (j == 0) client->dev_cheats.invisible = val;
                                        else if (j == 1) client->dev_cheats.invulnerable = val;
                                        else if (j == 2) client->dev_cheats.no_aggro = val;
                                        else if (j == 3) client->dev_cheats.no_wall_collision = val;
                                        else if (j == 4) client->dev_cheats.no_collision = val;
                                        else if (j == 5) client->dev_cheats.no_grid_influence = val;
                                        break;
                                    }
                                }
                            }
                        ...
                        char msg[100];
                        snprintf(msg, 99, "set flags: invisible=%d invulnerable=%d no_aggro=%d no_wall=%d no_collision=%d no_grid=%d",
                            !!client->dev_cheats.invisible, !!client->dev_cheats.invulnerable,
                            !!client->dev_cheats.no_aggro, !!client->dev_cheats.no_wall_collision,
                            !!client->dev_cheats.no_collision, !!client->dev_cheats.no_grid_influence);
                        broadcast_dev_message(this, client, msg);
                    ...
                    {
                        uint32_t target = atoi(arg1);
                        if (target >= 1 && target <= 500)
                        {
                            double total = 0;
                            for (uint32_t l = 1; l < target; l++)
                                total += xp_to_reach_level(l);
                            client->experience = total;
                            char msg[100];
                            snprintf(msg, 99, "set level to %u", target);
                            broadcast_dev_message(this, client, msg);
                        }
                    }
                }
        ...
        case rr_serverbound_dev_cheat:
        {
            ...
            if (!client->dev)
            {
                printf("dev cheat request by non-dev: %d\n", cheat_type);
                break;
            }
            ...
            case rr_dev_cheat_summon_mob:
            {
                ...
                {
                    char msg[100];
                    snprintf(msg, 99, "summoned %s %s x%d",
                             RR_MOB_NAMES[id], RR_RARITY_NAMES[rarity], count);
                    broadcast_dev_message(this, client, msg);
                }
            ...
            case rr_dev_cheat_flags:
            {
...

                {
                    char msg[100];
                    snprintf(msg, 99, "set flags: invisible=%d invulnerable=%d no_aggro=%d no_wall=%d no_collision=%d no_grid=%d",
                             !!client->dev_cheats.invisible,
                             !!client->dev_cheats.invulnerable,
                             !!client->dev_cheats.no_aggro,
                             !!client->dev_cheats.no_wall_collision,
                             !!client->dev_cheats.no_collision,
                             !!client->dev_cheats.no_grid_influence);
                    broadcast_dev_message(this, client, msg);
                }
            ...
            case rr_dev_cheat_speed_percent:
            {
                ...
                {
                    char msg[100];
                    snprintf(msg, 99, "set speed to %.0f%%", speed_percent * 100);
                    broadcast_dev_message(this, client, msg);
                }
                break;
            }
            case rr_dev_cheat_fov_percent:
            {
                ...
                {
                    char msg[100];
                    snprintf(msg, 99, "set FOV to %.0f%%", fov_percent * 100);
                    broadcast_dev_message(this, client, msg);
                }
                break;
            }
            case rr_dev_cheat_give_petal:
            {
                uint8_t id = proto_bug_read_uint8(&encoder, "id");
                uint8_t rarity = proto_bug_read_uint8(&encoder, "rarity");
                uint32_t count = proto_bug_read_varuint(&encoder, "count");
                if (id >= rr_petal_id_max || rarity >= rr_rarity_id_max || count == 0)
                    break;
                if (count > 999) count = 999;
                ...
                {
                    char msg[100];
                    snprintf(msg, 99, "gave self %u x %s %s",
                             count, RR_RARITY_NAMES[rarity], RR_PETAL_NAMES[id]);
                    broadcast_dev_message(this, client, msg);
                }
                break;
            }
