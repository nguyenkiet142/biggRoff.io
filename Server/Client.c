...
double CRAFT_XP_GAINS[rr_rarity_id_max - 1] = {1, 8, 60, 750, 25000, 1000000, 100000000, 10000000000, 1000000000000, 100000000000000};
                                  ...
                                  uint8_t rarity, uint32_t count)
{
    ...
    while (now >= 5)
    {
        ...
        double prob = base * (++this->craft_fails[id][rarity]);
        if (rarity >= rr_rarity_id_eternal)
            prob = fmin(prob, RR_BASE_CRAFT_CHANCES[rarity] + 0.30);
        if (id == rr_petal_id_basic || rr_frand() < prob)
        {
            ++success;
