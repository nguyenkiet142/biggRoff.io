    ...
    struct rr_petal_data const *petal_data)
{
    ...
    float holdingRadius = 50 + center_physical->radius;
                            ...
                            struct rr_component_player_info *player_info)
{
    ...
    for (uint64_t outer = 0; outer < player_info->slot_count; ++outer)
    {
        ...
        else if (data->id == rr_petal_id_cactus)
        {
            player_info->modifiers.max_health_bonus += 2000.0f * (slot->rarity + 1);
        }
        else if (data->id == rr_petal_id_cactus_golden)
        {
                          ...
                          struct rr_component_player_info_petal *p_petal)
{
    ...
    uint8_t petal_id = petal->id;
    uint8_t petal_rarity = petal->rarity;
    struct rr_component_player_info_petal_slot *petal_slot = petal->slot;

    rr_simulation_request_entity_deletion(simulation, p_petal->entity_hash);

    uint8_t m_id, m_rar;
    if (petal_id == rr_petal_id_egg)
    {
        m_id = rr_mob_id_trex;
        m_rar = petal_rarity >= 1 ? petal_rarity - 1 : 0;
    }
    else if (petal_id == rr_petal_id_hornet_egg)
    {
        m_id = rr_mob_id_hornet;
        m_rar = petal_rarity >= 1 ? petal_rarity - 1 : 0;
    }
    else if (petal_id == rr_petal_id_honeybee_egg)
    {
        m_id = rr_mob_id_honeybee;
        m_rar = petal_rarity >= 1 ? petal_rarity - 1 : 0;
    }
    else if (petal_id == rr_petal_id_meteor)
    {
        m_id = rr_mob_id_meteor;
        m_rar = petal_rarity >= 1 ? petal_rarity - 1 : 0;
    }
    else if (petal_id == rr_petal_id_stick)
    {
        m_id = rr_mob_id_sandstone;
        m_rar = petal_rarity >= 1 ? petal_rarity - 1 : 0;
    }
    ...
    uint8_t phys_arena = physical->arena;
    float phys_x = physical->x;
    float phys_y = physical->y;
    uint8_t relations_team = relations->team;
    EntityHash relations_nest = relations->nest;

    EntityIdx mob_id = rr_simulation_alloc_mob(
        simulation, phys_arena, phys_x, phys_y, m_id, m_rar,
        relations_team);
    ...
    rr_component_relations_set_team(mob_relations, relations_team);
    ...
    if (m_id == rr_mob_id_trex || m_id == rr_mob_id_hornet ||
        m_id == rr_mob_id_honeybee)
    {
        mob_relations->nest = relations_nest;
    ...
    {
        if (petal_slot != NULL)
            petal_slot->petals[1].entity_hash = RR_NULL_ENTITY;
    }
                                                    ...
                                                    void *simulation)
{
    ...
    // Track moon petal hash for orbit center redirection
    ...
    for (uint64_t outer = 0; outer < player_info->slot_count; ++outer)
    {
        ...
        for (uint64_t inner = 0; inner < slot->count; ++inner)
        {
            ...
            else
            {
                ...
                else if (data->id == rr_petal_id_stick)
                {
                    if (rr_simulation_has_petal(simulation, p_petal->entity_hash))
                        system_egg_hatching_logic(simulation, player_info, p_petal);
                }
