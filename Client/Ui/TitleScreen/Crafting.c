                                         ...
                                         struct rr_game *game)
{
    ...
    if (game->crafting_data.success_count ||
        game->crafting_data.count < PETALS_PER_CRAFT)
        attempts = game->crafting_data.last_attempts;
    double base = RR_BASE_CRAFT_CHANCES[game->crafting_data.crafting_rarity];
    double cap = game->crafting_data.crafting_rarity >= rr_rarity_id_eternal
                     ? fmin(1, base + 0.30)
                     : 1;
    double displayChance = fmin(base + attempts * 0.002, cap);
    double chance = 100 * displayChance;
                                     ...
                                     struct rr_game *game)
{
    ...
    case rr_rarity_id_eternal:
        data->text = "1t xp per craft";
        break;
    case rr_rarity_id_unique:
        data->text = "100t xp per craft";
        break;
    }
}
