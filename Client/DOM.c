...
uint32_t rr_dom_get_selection_start(char const *name)
{
    return EM_ASM_INT({
...
void rr_dom_set_selection_start(char const *name, uint32_t pos)
{
    EM_ASM({
        var el = document.getElementById(UTF8ToString($0));
        if (el) el.selectionStart = el.selectionEnd = $1;
    }, name, pos);
}

void rr_dom_set_style(char const *name, char const *prop, char const *value)
{
    EM_ASM({
        var el = document.getElementById(UTF8ToString($0));
        if (el) el.style[UTF8ToString($1)] = UTF8ToString($2);
    }, name, prop, value);
}
