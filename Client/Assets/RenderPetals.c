...
static void rr_draw_rounded_rock_polygon(struct rr_renderer *renderer,
                                          struct RockVertex *pts, int sides,
                                          float roundness, uint32_t fill,
                                          uint32_t stroke, float stroke_width)
{
    rr_renderer_set_fill(renderer, fill);
    ...
    {
        int next = (i + 1) % sides;
        mx[i] = (pts[i].x + pts[next].x) / 2.0f;
        my[i] = (pts[i].y + pts[next].y) / 2.0f;
    }

    int prev0 = sides - 1;
    float s0_x = pts[0].x + roundness * (mx[prev0] - pts[0].x);
    float s0_y = pts[0].y + roundness * (my[prev0] - pts[0].y);
    rr_renderer_move_to(renderer, s0_x, s0_y);

    for (int i = 0; i < sides; ++i)
    {
        int next = (i + 1) % sides;
        float ex = pts[i].x + roundness * (mx[i] - pts[i].x);
        float ey = pts[i].y + roundness * (my[i] - pts[i].y);
        float sx_next = pts[next].x + roundness * (mx[i] - pts[next].x);
        float sy_next = pts[next].y + roundness * (my[i] - pts[next].y);

        rr_renderer_quadratic_curve_to(renderer, pts[i].x, pts[i].y, ex, ey);
        rr_renderer_line_to(renderer, sx_next, sy_next);
    }
...
static void draw_procedural_rock(struct rr_renderer *renderer, float cx,
                                  float cy)
{
    const int sides = 5;
    const float radius = 20.0f;
    const float roughness = 0.00f;
    const float roundness = 0.00f;
    const float seed = 1.0f;

    struct RockVertex outerPts[16];
    struct RockVertex innerPts[16];

    for (int i = 0; i < sides; ++i)
    {
        float angle = (float)i * 2.0f * (float)M_PI / (float)sides;
        float offset = sinf((float)i * 1.7f + seed) *
                       cosf((float)i * 2.3f - seed * 0.5f);

        float r_out = radius * (1.0f + roughness * offset);
        outerPts[i].x = cx + r_out * cosf(angle);
        outerPts[i].y = cy + r_out * sinf(angle);

        float r_in = (radius * 0.80f) * (1.0f + roughness * offset);
        innerPts[i].x = cx + r_in * cosf(angle);
        innerPts[i].y = cy + r_in * sinf(angle);
    }

    rr_draw_rounded_rock_polygon(renderer, outerPts, sides, roundness,
                                 0xff59606b, 0xff393e47, 0.25f);
    rr_draw_rounded_rock_polygon(renderer, innerPts, sides, roundness,
                                 0xff767f8c, 0, 0.0f);
}
                            ...
                            uint8_t flags)
{
    ...
    else
    {
        ...
        case rr_petal_id_egg:
            ...
            rr_renderer_stroke(renderer);
            break;
        case rr_petal_id_hornet_egg:
            rr_renderer_set_stroke(renderer, 0xff5C2E00);
            rr_renderer_set_fill(renderer, 0xff8B4513);
            rr_renderer_set_line_width(renderer, 3);
            rr_renderer_begin_path(renderer);
            rr_renderer_arc(renderer, 0, 0, 12);
            rr_renderer_fill(renderer);
            rr_renderer_stroke(renderer);
            break;
        case rr_petal_id_honeybee_egg:
            rr_renderer_set_stroke(renderer, 0xffB8860B);
            rr_renderer_set_fill(renderer, 0xffDAA520);
            rr_renderer_set_line_width(renderer, 3);
            rr_renderer_begin_path(renderer);
            rr_renderer_arc(renderer, 0, 0, 12);
            rr_renderer_fill(renderer);
        ...
        case rr_petal_id_mint:
            ...
            break;
        case rr_petal_id_sawblade:
        {
            const int teeth = 20;
            const float outer_r = 18.0f;
            const float inner_r = 20.5f;
            const float slant = -0.90f;
            const float hole_r = 5.5f;
            const int hole_segments = 8;

            rr_renderer_set_fill(renderer, 0xff888888);
            rr_renderer_set_stroke(renderer, 0xff6f6f6f);
            ...
            {
                float angle = (float)i * M_PI / teeth;
                float r = (i % 2 == 0) ? outer_r : inner_r;
                if (i % 2 != 0)
                    angle += slant * (M_PI / teeth);
            ...
            for (int i = hole_segments; i >= 0; --i)
            {
                float angle = (float)i * 2.0f * M_PI / hole_segments;
                float x = hole_r * cosf(angle);
                float y = hole_r * sinf(angle);
                if (i == hole_segments)
                    rr_renderer_move_to(renderer, x, y);
                else
                    rr_renderer_line_to(renderer, x, y);
        ...
        {
            {
                ...
                rr_renderer_rotate(renderer, 0.29f);
                rr_renderer_set_fill(renderer, 0xff777777);
                rr_renderer_set_stroke(renderer, 0xff5F5F5F);
                rr_renderer_set_line_width(renderer, 4.0f);
                rr_renderer_set_line_join(renderer, 1);
                rr_renderer_begin_path(renderer);
                rr_renderer_move_to(renderer, 0.0f, -16.0f);
                rr_renderer_line_to(renderer, 15.22f, -4.94f);
                rr_renderer_line_to(renderer, 9.4f, 12.94f);
                rr_renderer_line_to(renderer, -9.4f, 12.94f);
                rr_renderer_line_to(renderer, -15.22f, -4.94f);
                rr_renderer_line_to(renderer, 0.0f, -16.0f);
                rr_renderer_fill(renderer);
                rr_renderer_stroke(renderer);
        ...
        case rr_petal_id_stick:
        {
            ...
            // Tỷ lệ kích thước chung
            const float stick_scale = 1.80f;

            // Lớp vỏ gỗ tối ngoài cùng
            rr_renderer_set_stroke(renderer, 0xff654b18);
            rr_renderer_set_line_width(renderer, 8.0f * stick_scale);
            
            rr_renderer_begin_path(renderer);
            rr_renderer_move_to(renderer, -5.00f * stick_scale, 10.00f * stick_scale);
            rr_renderer_line_to(renderer, 0.00f * stick_scale, 4.00f * stick_scale);
            rr_renderer_line_to(renderer, 8.00f * stick_scale, -4.00f * stick_scale);
            rr_renderer_stroke(renderer);

            rr_renderer_begin_path(renderer);
            rr_renderer_move_to(renderer, 0.00f * stick_scale, 4.00f * stick_scale);
            rr_renderer_line_to(renderer, -2.00f * stick_scale, -4.00f * stick_scale);
            rr_renderer_stroke(renderer);

            // Lớp lõi gỗ sáng bên trong
            rr_renderer_set_stroke(renderer, 0xff7d5a1f);
            rr_renderer_set_line_width(renderer, 3.0f * stick_scale);
        ...
        case rr_petal_id_moon:
            ...
            // --- SINH HỐ DOUBLE-DRAW (MERGED OUTLINES) ---
            // Bước 1: Vẽ viền rộng của tất cả các hố
        ...
        case rr_petal_id_cactus:
        {
            rr_renderer_set_fill(renderer, 0xff39C660);
            rr_renderer_set_stroke(renderer, 0xff2DA14C);
            rr_renderer_set_line_width(renderer, 5.5f);
            struct rr_renderer_context_state state_star;
            rr_renderer_context_state_init(renderer, &state_star);
            rr_renderer_translate(renderer, 0.0f, 0.0f);
            rr_renderer_scale(renderer, 0.5f);
            rr_renderer_begin_path(renderer);
            rr_renderer_move_to(renderer, 0.0f, -30.0f);
            rr_renderer_line_to(renderer, 10.0f, -23.0f);
            rr_renderer_line_to(renderer, 21.0f, -21.0f);
            rr_renderer_line_to(renderer, 23.0f, -10.0f);
            rr_renderer_line_to(renderer, 30.0f, 0.0f);
            rr_renderer_line_to(renderer, 23.0f, 10.0f);
            rr_renderer_line_to(renderer, 21.0f, 21.0f);
            rr_renderer_line_to(renderer, 10.0f, 23.0f);
            rr_renderer_line_to(renderer, 0.0f, 30.0f);
            rr_renderer_line_to(renderer, -10.0f, 23.0f);
            rr_renderer_line_to(renderer, -21.0f, 21.0f);
            rr_renderer_line_to(renderer, -23.0f, 10.0f);
            rr_renderer_line_to(renderer, -30.0f, 0.0f);
            ...
            rr_renderer_fill(renderer);
            rr_renderer_stroke(renderer);
            rr_renderer_context_state_free(renderer, &state_star);

            rr_renderer_set_fill(renderer, 0xff74D78F);
            rr_renderer_set_stroke(renderer, 0xff74D78F);
            rr_renderer_set_line_width(renderer, 2.0f);
            struct rr_renderer_context_state state_circle;
            rr_renderer_context_state_init(renderer, &state_circle);
            rr_renderer_translate(renderer, 0.0f, 0.0f);
            rr_renderer_scale(renderer, 0.2f);
            rr_renderer_begin_path(renderer);
            rr_renderer_arc(renderer, 0, 0, 30.0f);
            rr_renderer_fill(renderer);
            rr_renderer_stroke(renderer);
            rr_renderer_context_state_free(renderer, &state_circle);
            break;
        }
        case rr_petal_id_cactus_golden:
        {
            rr_renderer_set_fill(renderer, 0xffe6d450);
            rr_renderer_set_stroke(renderer, 0xffb8a940);
            rr_renderer_set_line_width(renderer, 5.5f);
            struct rr_renderer_context_state state_outer;
            rr_renderer_context_state_init(renderer, &state_outer);
            rr_renderer_translate(renderer, 0.0f, 0.0f);
            rr_renderer_scale(renderer, 0.53f);
            rr_renderer_begin_path(renderer);
            rr_renderer_move_to(renderer, 0.0f, -30.0f);
            rr_renderer_line_to(renderer, 10.0f, -23.0f);
            rr_renderer_line_to(renderer, 21.0f, -21.0f);
            ...
            rr_renderer_fill(renderer);
            rr_renderer_stroke(renderer);
            rr_renderer_context_state_free(renderer, &state_outer);

            rr_renderer_set_fill(renderer, 0xffede084);
            struct rr_renderer_context_state state_inner;
            rr_renderer_context_state_init(renderer, &state_inner);
            rr_renderer_translate(renderer, 0.0f, 0.0f);
            ...
            rr_renderer_line_to(renderer, -10.0f, -23.0f);
            rr_renderer_fill(renderer);
            rr_renderer_context_state_free(renderer, &state_inner);

            rr_renderer_set_fill(renderer, 0xfff8f4d3);
            rr_renderer_set_stroke(renderer, 0xfff8f4d3);
            rr_renderer_set_line_width(renderer, 2.0f);
            struct rr_renderer_context_state state_center;
            rr_renderer_context_state_init(renderer, &state_center);
            rr_renderer_translate(renderer, 0.0f, 0.0f);
            rr_renderer_scale(renderer, 0.15f);
            rr_renderer_begin_path(renderer);
            rr_renderer_arc(renderer, 0, 0, 30.0f);
            rr_renderer_fill(renderer);
            rr_renderer_stroke(renderer);
            rr_renderer_context_state_free(renderer, &state_center);
            break;
        }
        ...
        {
            {
                struct rr_renderer_context_state st0;
                rr_renderer_context_state_init(renderer, &st0);
                rr_renderer_rotate(renderer, 2.64f);

                // PHASE 1: oversized strokes (width ×2 = 4.0f) → builds full silhouette
                ...
                rr_renderer_set_line_join(renderer, 1);
                {
                    ...
                    rr_renderer_begin_path(renderer);
                    rr_renderer_arc(renderer, 0, 0, 7.37f);
                    rr_renderer_stroke(renderer);
                ...
                }
                {
                    ...
                    rr_renderer_begin_path(renderer);
                    rr_renderer_move_to(renderer, 0.0f, -17.0f);
                    rr_renderer_line_to(renderer, 6.8f, 0.0f);
                    rr_renderer_line_to(renderer, 0.0f, 17.0f);
                    rr_renderer_line_to(renderer, -6.8f, 0.0f);
                    rr_renderer_line_to(renderer, 0.0f, -17.0f);
                    rr_renderer_stroke(renderer);
                ...
                // PHASE 2: fills cover inner half of strokes → only outer border remains
                {
                    ...
                    rr_renderer_set_fill(renderer, 0xff9C1010);
                    rr_renderer_begin_path(renderer);
                    rr_renderer_arc(renderer, 0, 0, 7.37f);
                    rr_renderer_fill(renderer);
